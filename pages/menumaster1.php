 <?php 
include_once '../includes/db_connect.php';
include_once '../includes/functions.php';
include_once 'function/menufunction.php';
 require_once("header.php");
 
sec_session_start();

if (login_check() == true) {
    $logged = 'in';
} else {
    $logged = 'out'; header("Location:../includes/logout.php");
}
 
 $icode=$edm="";
if(!empty($_GET["icode"])){  $icode = $_GET["icode"];  }
if(!empty($_GET["edm"])){  $edm = $_GET["edm"];  }
 $psno = $cname= $status1= $status2="";
	
	 if($icode!=""){
	   if((!empty($edm)) && ($edm=="del")){   
		     $del_qry = "DELETE FROM menu_master WHERE trim(s_id)=trim('".$icode."')";
			 $result = mysql_query($del_qry);
	  	} 
	 
	 
	if((!empty($edm)) && ($edm=="edit")){   
	  $select_query =  "SELECT `s_id`, `menu_name`,`menu_head`,`menu_url`, `c_date`, `c_user`, `status` FROM `menu_master`  WHERE  s_id='".$icode."'";
	//echo $select_query;
	  $resultk = mysql_query($select_query);
	
    	while ($rowk = mysql_fetch_array($resultk)) {
			if($rowk["status"]=="A") $status1="selected";
			if($rowk["status"]=="I") $status2="selected";
			$psno = $rowk["s_id"];
			$cname = $rowk["menu_name"];
						$mhead = $rowk["menu_head"];
									$murl = $rowk["menu_url"]; 
    	}
	}
	}
 
 ?>
 <!DOCTYPE html>
<html>
<head> 
</head>
 	<!-- about-heading -->
	<div class="about-heading">
		<h2>MENU <span>MASTER </span></h2>
	</div>
	<!-- //about-heading -->
    
     <!-- choose -->
	<div class="choose jarallax">
  
		<div class="w3-agile-page">
			<div class="container">
              <div class="header">
              	<ul class="breadcrumb">
                	<li><a href="home.php">Home</a> </li>
                	<li class="active">Menu Master </li>
            	</ul>
        	</div> 
		 			<div class="col-md-3">
					 		<div class="top-choose-page">
							<div class="choose-info-top">
								<div class="choose-left-grid col-sm-12">
									<div class="choose-page-grid ">
									<?php  include_once("leftmenus.php");?>
                                    </div>
								</div>
								<div class="clearfix"> </div>
							</div>
						</div>
					</div>
					<div class="col-md-9 choose-grid1">
					 		<div class="agileinfo-contact-form-grid">
						 			 
                                 
                                        <!-- form start -->
                             <form  name="cform" method="post" action="menumaster1.php">
                                    
                                      <div class="col-sm-3 textalign labelalign"> MENU NAME
                                            <input type="text" class="form-control"  name="mname" id="mname" placeholder="Menu Name" maxlength="80"  onkeypress="return onlyAlphabets(event,this)"  value="<?php echo $cname; ?>" required/>
											<input type="hidden"   name="sid" id="sid" value="<?php echo $psno; ?>" />
                                           </div>
                                          <div class="col-sm-3 textalign labelalign">   MENU HEAD
                                         <select class="form-control"  name="mhead" id="mhead" required>
                                         <option value="0">-- Select --</option>
                                        <?php 
				$sqlquery2 = "select `s_id`, `menu_name` from menu_master where status='A' and menu_head='0'";
	        $queryresult2 = mysql_query($sqlquery2) or die("Query failed --1");
  			 
								while ($row2 = mysql_fetch_array($queryresult2))
  								{
									if($mhead==$row2["s_id"]) $dselval="selected";
									else  $dselval=" ";
									echo  "<option value='".$row2["s_id"]."' $dselval >".$row2["menu_name"]."</option>" ;
								} 
								?>
                                </select>  </div>
                                          <div class="col-sm-3 textalign labelalign">   MENU URL  
                           <input type="text" class="form-control"  name="murl" id="murl" placeholder="Menu URL" maxlength="80" value="<?php echo $murl; ?>" required/>
                                          </div>
                                          <div class="col-sm-3 textalign labelalign">  STATUS
                                           <select class="form-control"  name="status" id="status">
                                                <option value="A" <?php echo $status1; ?>>Active</option>
                                                <option value="I" <?php echo $status2; ?>>InActive</option>
                                               </select> 
                                        </div>
                                          <br /><br />
<br />

                                          <div class="col-sm-12" align="center"> 
                                   <button class="btn1" type="submit">Submit</button>
                                    <button type="reset" class="btn1" >clear</button>
                                  </div>
                                </form>
                
						</div>
    <?php 
if (isset($_POST['mname'], $_POST['status'])) {
    
     $mname =   make_safe($_POST['mname']);
     $mhead =   make_safe($_POST['mhead']);
	 $murl =   make_safe($_POST['murl']);
	$status =   make_safe($_POST['status']);
	 $userid =  htmlentities($_SESSION['USER_ID']);  
   $sid =  make_safe($_POST['sid']);
  // echo $mhead;
 	if ($mname!="" && $mhead!="" && $murl!="") {
	 
		if(menuname($mname,$mhead,$murl,$status) ){
				echo $mpass = menumaster($sid,$mname,$mhead,$murl,$status,$userid);
				//header("location:menumaster.php");
			}
			else{ echo 'This Menu Name already exits'; } 
 	} else {
    	// The correct POST variables were not sent to this page. 
    	echo 'Please select all the mandatory fields';
	}
} 
?>                   
                 
                   <br />
                         <div class="col-sm-12">
        <div class="panel panel-default">
			
			<div class="panel-body ">   <?php echo $sd =menugrid(); ?>     </div>
               </div></div>
					</div>
					<div class="clearfix"> </div>
				</div>
                
			</div>
         
	</div>
	<!-- //choose -->
 
    <?php require_once("footer.php"); ?>
 </body>
 </html>